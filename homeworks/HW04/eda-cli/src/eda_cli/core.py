...
def compute_quality_flags(
    ...
    df: Optional[pd.DataFrame] = None  # Добавим параметр для проверки эвристик
) -> Dict[str, Any]:
    """
    ...
    # НОВЫЕ ЭВРИСТИКИ - инициализируем переменные до использования
    ...
    many_zeros_count = 0
    suspicious_duplicates = False
    
    if df is not None:
        # Считаем количество константных колонок
        ...
        for col in summary.columns:
            if not col.is_numeric and col.unique > 50:  # Порог: более 50 уникальных значений
                high_cardinality_count += 1
        
        # Проверка дубликатов в колонках с 'id' в названии
        for col_name in df.columns:
            if 'id' in col_name.lower():
                if df[col_name].duplicated().any():
                    suspicious_duplicates = True
                    break
        
        # Проверка большого количества нулей в числовых колонках
        for col in summary.columns:
            if col.is_numeric:
                # Безопасно получаем колонку
                try:
    ...
    flags["has_suspicious_id_duplicates"] = suspicious_duplicates
    flags["has_many_zero_values"] = many_zeros_count > 0
    flags["many_zero_values_count"] = many_zeros_count

    # Обновляем интегральный скоринг с учётом новых эвристик
    score = 1.0
    score -= max_missing_share  # чем больше пропусков, тем хуже
    
    # Штрафы за новые эвристики
    if constant_columns_count > 0:
        score -= 0.1 * constant_columns_count
    if high_cardinality_count > 0:
        score -= 0.05 * high_cardinality_count
    if suspicious_duplicates:
        score -= 0.2
    if many_zeros_count > 0:
        score -= 0.15 * many_zeros_count
